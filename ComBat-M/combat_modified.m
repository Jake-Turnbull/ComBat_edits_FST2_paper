function [bayesdata,delta_star,gamma_star,t2,gamma_hat,delta_hat,a_prior,b_prior] = combat_modified(dat, batch, mod, parametric, NameValueArgs)
    ...
    batchmod = dummyvar(categorical(batch));
    ...
    if ~isempty(ReferenceBatch)
        ...
        var_pooled = var_ref; % features x 1
        % replace any zeros:
        if any(var_pooled == 0)
            ...
            if isempty(nonzero)
                ...
                var_pooled(var_pooled == 0) = median(nonzero);
            end
        end
    ...
    if size(design,2) > n_batch
        X_cov = design(:, n_batch+1:end);
        X_cov = X_cov - mean(X_cov, 1);
        B_cov = B_hat(n_batch+1:end, :);
    Cov_effects = (X_cov * B_cov)';   % [features x n_samples]
    else
        Cov_effects = zeros(size(dat));   % features x n_array
    end
    ...
    assert(size(gamma_hat,1) == n_batch, 'gamma_hat rows must equal n_batch');
    assert(size(gamma_hat,2) == size(dat,1), 'gamma_hat columns must equal n_features');
