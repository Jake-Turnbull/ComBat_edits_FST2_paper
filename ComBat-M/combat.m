...
% mod = [1 2 1 2 1 2 1 2 1];

function [bayesdata, stand_mean] = combat(dat, batch, mod, parametric)
    [sds] = std(dat')';
    wh = find(sds==0);
    [ns,ms] = size(wh);
    if ns>0
    ...
    % Least squares estimation of the batch and covariate effects of the
    % data
    ...
    %disp(['Size of gamma bar: ',num2str(size(gamma_bar))])

	t2 = var(gamma_hat');
    %disp(['Size of tau: ',num2str(size(t2))])

	delta_hat_cell = num2cell(delta_hat,2);
    %disp(['Size of delta_hat: ',num2str(size(delta_hat))])
